...
def login_form(request):
    if request.method=="POST":
        abc=AuthenticationForm(request,data=request.POST)
        if abc.is_valid():
            ...
            if user is not None:
                ...
                messages.error(request,'login not success full')
                
        else:
            messages.error(request,"login not sucessfull invalid") 
            
                   
    abc=AuthenticationForm()
    return render(request=request,template_name='login.html',context={'login_detail':abc})        

def logout_form(request):
    logout(request)
    messages.info(request,"logout succsessfull ")
    return redirect('login')    
